-- ============================================================
-- ARRANJO DE ORADORES — Supabase Setup
-- Congregação Alto da Colina
-- Execute este script no SQL Editor do Supabase
-- ============================================================

-- 1. ESBOÇOS (temas dos discursos)
...
CREATE TABLE IF NOT EXISTS congregacoes (
  id         SERIAL PRIMARY KEY,
  nome       TEXT NOT NULL,
  dia        TEXT DEFAULT 'Domingo',
  hora       TEXT DEFAULT '9:00',
...
-- 4. VISITANTES (oradores que vieram pregar aqui)
CREATE TABLE IF NOT EXISTS visitantes (
  id                  SERIAL PRIMARY KEY,
  cong                TEXT NOT NULL,
  dia                 TEXT DEFAULT 'Sábado',
  data                TEXT NOT NULL,
  hora                TEXT DEFAULT '19:00',
  orador              TEXT NOT NULL,
  esboco_id           INTEGER REFERENCES esbocos(id) ON DELETE SET NULL,
  congregacao_local   TEXT DEFAULT 'Alto da Colina',
  endereco            TEXT DEFAULT '',
  relatorio_id        TEXT DEFAULT '',
  status              TEXT DEFAULT 'pendente'
                        CHECK (status IN ('confirmado','pendente','cancelado','realizado')),
  criado_em           TIMESTAMPTZ DEFAULT NOW()
);

-- 5. SAÍDAS (nossos oradores em outras congregações)
CREATE TABLE IF NOT EXISTS saidas (
  id           SERIAL PRIMARY KEY,
  data         TEXT NOT NULL,
  cong         TEXT NOT NULL,
  orador_id    INTEGER REFERENCES oradores(id) ON DELETE SET NULL,
  orador_nome  TEXT DEFAULT '',
  esboco_id    INTEGER REFERENCES esbocos(id) ON DELETE SET NULL,
  status       TEXT DEFAULT 'pendente'
                 CHECK (status IN ('confirmado','pendente','cancelado','realizado')),
  criado_em    TIMESTAMPTZ DEFAULT NOW()
...
-- ============================================================
-- SEGURANÇA: Habilitar RLS e permitir acesso público
-- (ajuste conforme necessário para autenticação futura)
-- ============================================================
...
-- Políticas de acesso público (leitura e escrita sem autenticação)
CREATE POLICY "acesso_publico_esbocos"      ON esbocos      FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "acesso_publico_oradores"     ON oradores     FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "acesso_publico_congregacoes" ON congregacoes FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "acesso_publico_visitantes"   ON visitantes   FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "acesso_publico_saidas"       ON saidas       FOR ALL USING (true) WITH CHECK (true);

-- ============================================================
-- DADOS INICIAIS — Esboços (S-99-T completo)
-- ============================================================
INSERT INTO esbocos (n, tema) VALUES
...
-- ORADORES INICIAIS
-- ============================================================
INSERT INTO oradores (nome, cel, esboco_ids, status) VALUES
('Aurimar Erasmo de Oliveira',   '1999887766',    ARRAY(SELECT id FROM esbocos WHERE n IN (10,65)),         'confirmado'),
('Alison Oliveira',              '19993553927',   ARRAY(SELECT id FROM esbocos WHERE n IN (67,137)),        'confirmado'),
('Antonio José',                 '',              ARRAY(SELECT id FROM esbocos WHERE n IN (21,43,144)),     'confirmado'),
('Crystyano Santos',             '19996035529',   ARRAY(SELECT id FROM esbocos WHERE n IN (29,172,185)),    'confirmado'),
('Darcy Sousa',                  '19997980122',   ARRAY(SELECT id FROM esbocos WHERE n IN (4,7,35,88)),     'confirmado'),
('Diógenes Lima',                '11979562733',   ARRAY(SELECT id FROM esbocos WHERE n IN (90,154,160,164,192,193,194)), 'confirmado'),
('Helio Oliveira',               '',              ARRAY(SELECT id FROM esbocos WHERE n IN (15,160)),        'confirmado'),
('José Rogério',                 '19988107486',   ARRAY(SELECT id FROM esbocos WHERE n IN (14,37,56,83,88,95,99,117,119)), 'confirmado'),
('Marco Munhoz Jr',              '',              ARRAY(SELECT id FROM esbocos WHERE n IN (20,57,58,122,125,137,192)), 'confirmado'),
('Marcelo Carneiro Munhoz',      '19996110071',   ARRAY(SELECT id FROM esbocos WHERE n IN (26,86)),         'confirmado'),
('Saul Gonçalvez',               '19992147638',   ARRAY(SELECT id FROM esbocos WHERE n IN (1,10,18,24,50,53,158,183)), 'confirmado'),
('Alfredo Sthall',               '19992169214',   ARRAY(SELECT id FROM esbocos WHERE n IN (56,190)),        'confirmado'),
('Claudio Gazola',               '19982001684',   ARRAY(SELECT id FROM esbocos WHERE n IN (90)),            'pendente'),
('João Vitor Zacarias dos Santos','11914780604',  ARRAY(SELECT id FROM esbocos WHERE n IN (73,103,108,181)), 'confirmado'),
('Marcelo Milhan',               '19998501724',   ARRAY(SELECT id FROM esbocos WHERE n IN (2,3,7,10,15,16,17,24,29,52,73,74,104,109,116,119,121,142,150,165,168,169,173,176,192,193)), 'confirmado'),
('Vinicius Urbano',              '19992083150',   ARRAY(SELECT id FROM esbocos WHERE n IN (32)),            'confirmado'),
('Leonardo Molina',              '11984981365',   ARRAY(SELECT id FROM esbocos WHERE n IN (9,11,43,79,92,94,108,122,141,158,160,166,169)), 'confirmado');

-- ============================================================
-- FIM DO SCRIPT
-- Próximos passos:
-- 1. Acesse supabase.com e crie sua conta
-- 2. Crie um novo projeto
-- 3. Vá em SQL Editor e cole este script
-- 4. Clique em Run
-- 5. Copie a Project URL e a anon key de:
